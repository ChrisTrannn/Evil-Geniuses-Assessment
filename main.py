...
if __name__ == '__main__':
    ...
    if game_state.is_common_strategy(team, side):
        print(f"Entering via the light blue boundary is a common strategy used by {team} on {side} side.")
    else:
        print(f"Entering via the light blue boundary is not a common strategy used by {team} on {side} side.")
    
    # b. What is the average timer that Team2 on T (terrorist) side enters BombsiteB with least 2 rifles or SMGs?
    team, side, bombsite, weapons, min_num = 'Team2', 'T', 'BombsiteB', ['Rifle', 'SMG'], 2
    print(f'The average time that {team} on {side} side enters {bombsite} with at least {min_num} rifles or SMGs is: {game_state.average_timer(team, side, bombsite, weapons, min_num)}')
